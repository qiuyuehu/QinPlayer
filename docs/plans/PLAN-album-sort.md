...
| 把字段、方向拆成三个常驻按钮 | header 占用过宽，视觉上像一排设置按钮 | 四个选项收进一枚“排序：专辑名 · 升序”菜单触发器 |
...
- 触发器始终显示当前组合，例如 `排序：歌手 · 降序`，无需打开菜单也能确认状态。
...
1. `songs:getAll` 返回后，菜单触发器显示 `排序：专辑名 · 升序`，卡片 DOM 顺序符合名称升序。
2. 打开菜单选择“歌手”后，触发器显示 `排序：歌手 · 升序`，卡片改为歌手顺序。
...
1. 默认关闭，触发器显示 `排序：专辑名 · 升序`，`aria-expanded=false`。
